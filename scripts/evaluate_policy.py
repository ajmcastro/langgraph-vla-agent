...
def _run_mock(args: argparse.Namespace) -> None:
    ...
    print()
    print("  What this measures: software behavior — graph routing, executor logic,")
    print("  metric computation. The policy outputs a fixed zero-like action every step.")
    print("  L1/L2 values reflect distance from the synthetic fixture actions, not model quality.")
    print("  This mode proves the evaluation infrastructure is correct, not that any")
    print("  model performs well on a task.")
    print()
...
def _run_replay(args: argparse.Namespace) -> None:
    ...
    print()
    print("  What this measures: a sanity check — ReplayRobotPolicy reproduces the")
    print("  recorded actions exactly, so L1 error should be ~0. Non-zero L1 would")
    print("  indicate a bug in the evaluator or episode loading.")
    print("  This does NOT measure model quality. It verifies that the evaluation")
    print("  infrastructure round-trips episode data correctly.")
    print()
...
def _run_vla(args: argparse.Namespace) -> None:
    ...
    print()
    print("  What this measures: how closely the model's action predictions match")
    print("  the recorded actions in 3 synthetic fixture episodes (hand-crafted JSON,")
    print("  not real robot data from svla_so100_pickplace). Lower L1/L2 is better,")
    print("  but the absolute numbers are only meaningful relative to another checkpoint")
    print("  evaluated on the same episodes (e.g. make compare-checkpoints-vla).")
    print("  This does NOT measure closed-loop task success — the environment cannot")
    print("  simulate what happens if the model takes a different action than recorded.")
    print()
